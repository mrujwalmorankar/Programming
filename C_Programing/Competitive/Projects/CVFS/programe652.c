...
{
printf("---------------------------------------------------------------\n");
printf("---------------- Marvellus CVFS help page ---------------------\n");
printf("---------------------------------------------------------------\n");
...
{
printf("About : It is used to list clear the terminal\n");
printf("Usage : clear\n");
}
else if(strcmp(Name, "creat") == 0)
{
printf("About : It is used to list create new file\n");
...
{
printf("About : It is used to delete\n");
...
while(temp != NULL)
{
    if(strcmp(temp->FileName, name) == 0)   // navachi file ahe
    {
        bFlag = true;
        break;
    }
...
uareaobj.UFDT[i]->ptrinode->Buffer = (char *)malloc(MAXFILESIZE);           // IMP DATA chi jaga allocate zali
...
printf("---------------------------------------------------------\n");
printf("----------- Marvellus CVFS files Information ------------\n");
printf("---------------------------------------------------------\n");
...
for(i = 0; i < MAXOPENFILES; i++)
{
    if(uareaobj.UFDT[i] != NULL)
    {
        ...
        {
            //Deallocate memory of Bufffer
            free(uareaobj.UFDT[i]->ptrinode->Buffer);

            uareaobj.UFDT[i]->ptrinode->Buffer = NULL;

            strcpy(uareaobj.UFDT[i]->ptrinode->FileName, "");
...
//                          Data that we want to write
                            // size of data                       
//  Output :                //Number of bytes Sucessfully return
//  Author :                Ujwal Dipak Morankar
//  Date :                  2/08/2026
//
//////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////
//
//  Entry Point function of the CVSF project
//
//////////////////////////////////////////////////////////////////////
...
//Tokanize Command
char Command[5][20] = {{'\0'}};
int iCount = 0, iRet = 0 ,size=0;

char *Emptybuffer=NULL;
StartAuxillaryDataInitialisation();

printf("---------------------------------------------------------\n");
printf("----------- Marvellus CVFS started successfully ---------\n");
printf("---------------------------------------------------------\n");
...
while(1)
{
    ...
    if(iCount == 1)
    {
        ...
        if(strcmp(Command[0], "exit") == 0)
        {
            ...
            printf("Dealloating all resources of Marvellous CVFS\n");
    ...
    else if(iCount == 2)
    {
        ...
        else if(strcmp(Command[0], "write") == 0)
        {
            ...
            if(iRet=ERR_INVALID_PARAMETER)
            {
                  ...
                  printf("Plz refer man page for more information\n");
            }
            ...
            {
                printf("Error : There are no permission to write the data\n");
            }
             else if(iRet == ERR_INSUFFICIENT_SPACE)
            {
                printf("Error : There are no space to write the data\n");
            }
            else
            {
                printf("%d  bytes gets returns successfully written into the file\n",iRet);
            }
        }
        else
        {
